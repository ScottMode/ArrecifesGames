...
var goodFishGroup = null;
var badFishGroup = null;
var cruiseShip = null;
...
BasicGame.GameF.prototype = {
	...
	create: function () {
        ...
        //Cruise ship
        cruiseShip = this.add.sprite(this.world.centerX + 400, this.world.centerY - 20, 'cruiseShip');
        cruiseShip.anchor.setTo(0.5);
        cruiseShip.scale.setTo(0.3);
        this.physics.arcade.enable(cruiseShip);
        
        
        //Good fish
        goodFishGroup = this.add.physicsGroup();
        for (var i = 1; i < 9; i++) {
            
            var cor = null;
            
            var randX = Math.floor(Math.random() * (this.world.width));
            var randY = this.world.centerY + 100 + Math.floor(Math.random() * 400);
            
            if (i < 5) {
                cor = goodFishGroup.create(randX, randY, 'goodFish1');
            } 
            else if (i < 7) {
                cor = goodFishGroup.create(randX, randY, 'goodFish2');
            }
            else if (i < 9) {
                cor = goodFishGroup.create(randX, randY, 'goodFish3');
            }
            
            cor.scale.setTo(0.2);
            cor.anchor.setTo(0.5);
            cor.inputEnabled = true;
            cor.input.enableDrag(true);
        }
        
        
        //Bad fish
        badFishGroup = this.add.physicsGroup();
        for (var i = 1; i < 9; i++) {
            ...
            if (i < 5) {
                cor = badFishGroup.create(randX, randY, 'badFish1');
            } 
            else if (i < 7) {
                cor = badFishGroup.create(randX, randY, 'badFish2');
            }
            else if (i < 9) {
                cor = badFishGroup.create(randX, randY, 'badFish2');
            }
	...
	update: function () {
        ...
        if (gameState == "play"){
            ...
            this.physics.arcade.overlap(cruiseShip, goodFishGroup, this.gotGoodFish, null, this);
            this.physics.arcade.overlap(cruiseShip, badFishGroup, this.gotBadFish, null, this);
    ...
    gotGoodFish: function (player, object) {
        addScore(100);
        object.destroy();
    },
    
    gotBadFish: function (player, object) {
        addScore(-100);
        object.destroy();
    }
};